/* LyricSlide Pro - Core Logic v18 (Final Table-Safe Integration) */
...
const App = {
    ...
    init() {
        ...
        console.log("App Initialized. Version 18.0 (Coordinate Safe)");
    },
    ...
    async loadForPreview(file) {
        try {
            ...
            for (const path of slideFiles) {
                ...
                const slideData = [];
                // UPDATED: Match both standard shapes and table cells
    ...
    // --- THE RE-ENGINEERED TABLE FILLER ---
    lockInStyleAndReplace(xml, placeholder, replacement) {
        // FUZZY MATCH: Finds placeholder even if broken by XML tags or All-Caps
        ...
        return xml.replace(/<(p:sp|p:graphicFrame)>([\s\S]*?)<\/\1>/g, (fullFrame) => {
            ...
            if (phRegex.test(fullFrame)) {
                // Determine template font styles
                ...
                // Simple replacement for non-lyric items
    ...
    async loadDefaultTemplates() {
        ...
        try {
            ...
            names.forEach(name => {
                ...
                card.onclick = async () => { card.style.opacity = '0.5'; const b = await (await fetch(`./${encodeURIComponent(name)}`)).blob(); this.selectTemplate({name, file: new File([b], name)}, card); card.style.opacity = '1'; };
                grid.appendChild(card);
            });
            gallery.innerHTML = ''; gallery.appendChild(grid);
        } catch (e) { gallery.innerHTML = 'Template library not found.'; }
    },

    selectTemplate(item, el) { this.selectedTemplateFile = item.file; document.querySelectorAll('.template-card').forEach(c => c.classList.remove('selected')); el.classList.add('selected'); document.getElementById('selectedTemplateInfo').classList.remove('hidden'); document.getElementById('selectedTemplateName').textContent = item.name; },
