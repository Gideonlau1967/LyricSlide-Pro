/* LyricSlide Pro - Version 4.2.0 (Absolute Full Production Build) */
...
const App = {
    ...
    init() {
        ...
        if (alignSelect) {
            alignSelect.addEventListener('change', () => {
                if (this.originalSlides.length > 0) this.updatePreview(parseInt(this.elements.semitoneDisplay.textContent) || 0);
            });
        }
        this.theme.init();
    ...
    // --- CORE GENERATION (FULL MANIFEST REBUILD) ---
    async generate() {
        ...
        try {
            ...
            // Rebuild Registries via String (Strict Integrity)
    ...
    transposeParagraphs(xml, semitones) {
        return xml.replace(/<a:p[^>]*>([\s\S]*?)<\/a:p>/g, (matchFull, pXml) => {
            ...
            for (let i = 0; i < transposedLogic.length; i++) {
                ...
                if (newChar === "\n") {
                    newRuns += "<a:br/>";
                    while(metaIdx < charMeta.length && !charMeta[metaIdx].isBr) metaIdx++;
                    metaIdx++; continue;
                }
    ...
    async loadForPreview(file) {
        try {
            this.showLoading('Analyzing Slides...');
            const zip = await JSZip.loadAsync(file);
