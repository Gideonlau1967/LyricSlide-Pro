/* LyricSlide Pro - Core Logic v12.1 (Presenter Notes Integrated) */
...
const App = {
    ...
    init() {
        ...
        console.log("App Initialized. Version 15.1 (Notes Support)");
    },
    ...
    async generate() {
        ...
        try {
            ...
            // --- NOTES SETUP ---
            let templateNotesXml = null;
            if (templateRelsXml) {
                const notesMatch = templateRelsXml.match(/Type="[^"]+notesSlide"[^>]+Target="\.\.\/notesSlides\/(notesSlide\d+\.xml)"/);
                if (notesMatch) {
                    const notesPath = `ppt/notesSlides/${notesMatch[1]}`;
                    if (zip.file(notesPath)) templateNotesXml = await zip.file(notesPath).async('string');
                }
            }
            ...
            for (let i = 0; i < sections.length; i++) {
                ...
                // Handle Slide Rels & Notes
                let currentRels = templateRelsXml;
                if (templateNotesXml) {
                    const notesName = `notes_gen_${i + 1}.xml`;
                    const updatedNotes = this.updateNotesXml(templateNotesXml, sectionText);
                    newZip.file(`ppt/notesSlides/${notesName}`, updatedNotes);
                    currentRels = currentRels.replace(/Target="\.\.\/notesSlides\/notesSlide\d+\.xml"/, `Target="../notesSlides/${notesName}"`);
                }
                if (currentRels) newZip.file(`ppt/slides/_rels/${name}.rels`, currentRels);
    ...
    updateNotesXml(notesXml, text) {
        ...
        if (notesXml.includes('[Lyrics and Chords]')) {
            const phRegex = new RegExp(this.getPlaceholderRegexStr('[Lyrics and Chords]'), 'gi');
            return notesXml.replace(phRegex, () => `</a:t></a:r><a:r>${style}<a:t xml:space="preserve">${injected}</a:t></a:r><a:r>${style}<a:t xml:space="preserve">`);
        } else {
            // Fallback: Inject into first paragraph
            return notesXml.replace(/<a:p>([\s\S]*?)<\/a:p>/, `<a:p><a:r>${style}<a:t xml:space="preserve">${injected}</a:t></a:r></a:p>`);
        }
    },
    ...
    async transpose() {
        ...
        const titleFont = getFont('fontTitle'), titleSize = getSz('fontSizeTitle');
        const lyricsFont = getFont('fontLyrics'), lyricsSize = getSz('fontSizeLyrics');
        const copyFont = getFont('fontCopyright'), copySize = getSz('fontSizeCopyright');
    ...
    applyFontToShapeXml(shapeXml, fontFamily, fontSizeHundredths) {
        ...
        shapeXml = shapeXml.replace(/<a:rPr([^>]*)>([\s\S]*?)<\/a:rPr>/g, (_, attrs, inner) => {
            const newAttrs = this.applyFontSizeToAttrs(attrs, fontSizeHundredths);
            if (fontFamily) {
                inner = inner.replace(/<a:(latin|ea|cs)[^>]*(\/>|>[\s\S]*?<\/a:\1>)/g, '');
            }
    ...
    transposeLine(text, semitones) {
        ...
        return text.split('\n').map(line => {
            ...
            for (const m of matches) {
                const originalChord = m[0], pos = m.index + offset;
                ...
                if (diff > 0) {
                    let spaces = result.substring(pos + newChord.length).match(/^ +/);
                    if (spaces && spaces[0].length >= diff) result = result.substring(0, pos + newChord.length) + result.substring(pos + newChord.length + diff);
                    else offset += diff;
                } else if (diff < 0) {
        ...
        return xml.replace(/<p:sp>[\s\S]*?<\/p:sp>/g, (shapeXml) => {
            if (phRegex.test(shapeXml)) {
                ...
                if (placeholder === '[Lyrics and Chords]' && lines.length > 10) {
                    ...
                    if (szMatch) style = style.replace(/sz=\"\d+\"/, `sz="${Math.floor(parseInt(szMatch[1]) * Math.max(0.6, 1 - (lines.length - 10) * 0.05))}"`);
                }
                return shapeXml.replace(phRegex, () => `</a:t></a:r><a:r>${style}<a:t xml:space="preserve">${injected}</a:t></a:r><a:r>${style}<a:t xml:space="preserve">`)
                    .replace(/<a:t xml:space="preserve"><\/a:t>/g, '').replace(/<a:r><a:rPr[^>]*><a:t xml:space="preserve"><\/a:t><\/a:r>/g, '')
                    .replace('</a:bodyPr>', '<a:normAutofit fontScale="75000" lnSpcReduction="15000"/></a:bodyPr>');
            }
            return shapeXml;
        });
    },
    ...
    syncPresentationRegistry(newZip, presXml, presRelsXml, generated) {
        ...
        let ctEntries = generated.map(s => {
            ...
            entries += `<Override PartName="/ppt/notesSlides/notes_gen_${s.name.match(/\d+/)[0]}.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.notesSlide+xml"/>`;
            return entries;
        }).join('');
        const baseOverrides = '<Override PartName="/ppt/presentation.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.presentation.main+xml"/><Override PartName="/ppt/viewProps.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.viewProps+xml"/><Override PartName="/ppt/tableStyles.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.tableStyles+xml"/><Override PartName="/ppt/presProps.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.presProps+xml"/><Override PartName="/ppt/theme/theme1.xml" ContentType="application/vnd.openxmlformats-officedocument.theme+xml"/><Override PartName="/ppt/slideMasters/slideMaster1.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.slideMaster+xml"/><Override PartName="/ppt/slideLayouts/slideLayout1.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.slideLayout+xml"/><Override PartName="/ppt/notesMasters/notesMaster1.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.notesMaster+xml"/>';
        newZip.file('[Content_Types].xml', ctXml + ctEntries + baseOverrides + '</Types>');
    },
