/* LyricSlide Pro - Version 3.1.0 (MS Compliance Integrated) */
...
const App = {
    ...
    // --- MS COMPLIANCE: CONTENT TYPE REGISTRY ---
    ...
    // --- REPLACEMENT ENGINE (NO NESTED TAGS) ---
        ...
        return xml.replace(/<a:p>([\s\S]*?)<\/a:p>/g, (pMatch) => {
            ...
            for (let i = 0; i < rawLines.length; i++) {
                ...
                if (this.isChordLine(line) && next && !this.isChordLine(next) && !next.trim().startsWith('[')) {
                    ...
                    if (!text) newParagraphs += `<a:p><a:pPr algn="${alignment}"/></a:p>`;
                    else {
    ...
    async transpose() {
        ...
        const semitones = parseInt(document.getElementById('semitoneDisplay').textContent) || 0;
