/* 
   LyricSlide Pro - Version 3.4.1 (ULTRA-STABLE MERGED)
   - 100% Features from Version 3.0 (Transposition, Zoom, Preview, UI)
   - 100% Fixes for MS Office Repair Error (Registry Purge, Notes Master Link)
*/

const App = {
    version: "Version 3.4.1 - Production",
    elements: {
    ...
    async generate() {
        ...
        try {
            ...
            // INVESTIGATED FIX: Capture Notes Master Linkage
            let notesMasterRel = "";
            if (templateNotesPath) {
                ...
                const m = notesRelsXml.match(/<Relationship[^>]+Type="[^"]+notesMaster"[^>]*\/>/);
                notesMasterRel = m ? m[0] : "";
            }

            // CLEANUP: Purge physical slide files to prevent repair ghosts
            ...
            for (let i = 0; i < sections.length; i++) {
                ...
                if (templateNotesXml) {
                    ...
                    // Fixed Notes Rel with both Slide and NotesMaster link
    ...
    // --- TRANSPOSITION ENGINE (SURGERY LOGIC) ---
    async transpose() {
