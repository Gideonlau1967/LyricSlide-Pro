/* LyricSlide Pro - generate.js (v21.6 - Robust Placeholder & Table Engine) */
...
const App = {
    ...
    async generate() {
        ...
        try {
            ...
            // Templates use the very first slide found
            ...
            for (let i = 0; i < sections.length; i++) {
                // Replacing specific bracketed placeholders
            ...
            this.hideLoading(); 
            alert("Error during generation. See console for details."); 
        }
    },

    lockInStyleAndReplace(xml, placeholder, replacement) {
        // Advanced Regex to find text even if split by XML tags: [ T<tag/>I<tag/>T<tag/>L<tag/>E ]
        ...
        return xml.replace(/<(p:sp|p:graphicFrame)>([\s\S]*?)<\/\1>/g, (fullFrame, tagName, innerContent) => {
            ...
            if (phRegex.test(innerContent)) {
                ...
                // --- OPTION A: REGULAR TEXT BOX (TITLE / COPYRIGHT) ---
                if (!/LYRICS/i.test(placeholder)) {
                    ...
                    // We replace the matched fuzzy placeholder with our formatted text
                    return `<${tagName}>${innerContent.replace(phRegex, escapedText)}</${tagName}>`;
                }

                // --- OPTION B: TABLE (LYRICS AND CHORDS) ---
                const lines = (replacement || '').split(/\r?\n/);
                let tableRowsXml = '';
                lines.forEach((line) => {
                    ...
                    if (trimmed === '') { 
                        ...
                        // Fixed Chord Style: Courier New / Size 16
        ...
        try {
            ...
            names.forEach(name => {
                ...
                card.onclick = async () => {
                    ...
                    if (this.elements.selectedTemplateDisplay) this.elements.selectedTemplateDisplay.textContent = `Active Template: ${name.replace('.pptx','')}`;
                    const r = await fetch(`./${encodeURIComponent(name)}`);
                    const blob = await r.blob();
                    this.selectedTemplateFile = blob; // JSZip accepts Blobs directly
                };
                gallery.appendChild(card);
            });
        } catch (e) { gallery.innerHTML = 'Template loading error.'; }
    },
